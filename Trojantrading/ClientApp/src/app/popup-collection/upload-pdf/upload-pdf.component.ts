  ...
  uploadPdf(): void {
    if(this.file){
      ...
      if(_.lowerCase(this.file.name.split('.')[1]) != "pdf"){
        this.shareService.showError(".uploadpdf", "Please upload a pdf file", "left");
      }else{
        this.loadContent = false;
        this.fileService.SavePdf(this.selectedRole, this.formData).subscribe((res: ApiResponse) => {
          if (res.status == "success") {
            this.shareService.showSuccess(".uploadpdf", res.message, "left");
            setTimeout(() => {
            ...
            this.loadContent = true;
            this.shareService.showError(".uploadpdf", res.message, "left");
          }
        },
          (error: any) => {
            console.info(error);
      ...
      this.shareService.showError(".uploadpdf", "Upload file is empty", "left");
    }
  }
