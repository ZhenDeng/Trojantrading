    ...
    if (this.file) {
      ...
      this.fileService.SaveImage(this.formData).subscribe((res: ApiResponse) => {
        if (res.status == "success") {
          this.shareService.showSuccess(".uploadimg", res.message, "right");
          setTimeout(() => {
          ...
          this.loadContent = true;
          this.shareService.showError(".uploadimg", res.message, "right");
        }
      },
        (error: any) => {
          console.info(error);
      ...
      this.shareService.showError(".uploadimg", "Upload file is empty", "right");
    }
  }
